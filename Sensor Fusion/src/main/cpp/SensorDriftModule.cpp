...
void SensorDriftModule::updateSensors() {
  ...
  frc::SmartDashboard::PutNumber("drifted encoder theta", theta);
  ...
  /* Part 2: Drifting Sensors to match Magnetometer heading */
...
double SensorDriftModule::sensorDriftAlgorithm(double driftedSensor, double leadSensor, double lastTime) {

  // observed noise of magnetometer and max drift of gyro 
  double const noise_p = 0.02; 
  double const max_drift_per_sec = 0.4; 

  double error_delta = leadSensor - driftedSensor;
  double error_bound = noise_p * fabs(leadSensor);
