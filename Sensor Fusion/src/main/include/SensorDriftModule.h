...
class SensorDriftModule {
 public:
  ...
  SensorDriftModule(rev::CANSparkMax* lMotor, rev::CANSparkMax* rMotor); 
  ...
  void startTimer();
  //rev::SparkMaxRelativeEncoder getEncoderObj(std::unique_ptr<rev::SparkMaxRelativeEncoder> encoder);
